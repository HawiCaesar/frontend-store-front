...
const postResource = (path, data, requestOptions) => axios
.post(
  `http://localhost:4001${path}`, data, requestOptions
)
.then((response) => response.data)

const getResource = (path, requestOptions) => axios
.get(`http://localhost:4001${path}`, requestOptions)
.then((response) => response.data)


const Options = (user) => {
  const hasToken = Object.keys(user).length === 0 && user.hasOwnProperty('token')
  return  {
    headers: {
...
export const AuthProvider = ({children}) => {
    ...
    const accountCheck = useCallback(
        async() => {
            try {
